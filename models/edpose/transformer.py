...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        self.decoder_class_detach = "None"
    ...
    def detach_class_token(self, tensor):
        if self.decoder_class_detach == "detach":
            return tensor.detach()
        if self.decoder_class_detach == "clone":
            return tensor.clone()
        if self.decoder_class_detach == "detach_clone":
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if layer_id == self.num_box_decoder_layers - 1:
                ...
                if self.seperate_token_for_class:
                    new_reference_points_for_class = self.detach_class_token(torch.gather(new_reference_points[effect_num_dn:], 0, topk_proposals.unsqueeze(-1).repeat(1, 1, 4))) #.clone()
                    new_output_for_class = self.detach_class_token(torch.gather(output[effect_num_dn:], 0, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model)))#.clone())
                    # should i do clone or  detach? no i think
                ###
